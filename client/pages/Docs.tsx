...
export default function Docs() {
  useEffect(() => {
    setPageMeta({
      ...
      description:
        "Complete guide to BizPaySol ACH API. 5-minute quickstart, REST endpoints, webhooks, authentication, error codes, and SDK documentation.",
      ...
      ogDescription:
        "Integrate BizPay Solutions in minutes with our developer-friendly ACH API.",
    });
      ...
      <section className="bg-gradient-to-b from-blue-50 to-white py-12">
        <div className="container grid gap-8 md:grid-cols-[260px_1fr]">
          ...
          <div className="prose prose-slate max-w-none" id="quickstart">
            ...
            <pre>
              <code>{`curl -X POST https://api.bizpaysol.com/v1/customers \
  -H "Authorization: Bearer sk_test_123..." \
  -H "Content-Type: application/json" \
  -d '{"name":"Acme Inc","email":"ap@acme.com"}'`}</code>
            </pre>
            <p className="mt-2">Or with fetch (Node 18+):</p>
            <pre>
              <code>{`const res = await fetch('https://api.bizpaysol.com/v1/customers', {
  method: 'POST',
  headers: {
...
const data = await res.json();`}</code>
            ...
            <p className="mt-2 text-muted-foreground max-w-2xl">
              Integrate BizPay Solutions in minutes. Secure, NACHA‑compliant ACH
              with modern APIs, idempotency, and signed webhooks.
            </p>

            <h3>Getting Started</h3>
            <ul>
              <li>
                <strong>Base URL:</strong> https://api.bizpaysol.com/v1
              </li>
              <li>
                <strong>Auth:</strong> Send a Bearer API key in the
                Authorization header
              </li>
              <li>
                <strong>Idempotency:</strong> Include a unique Idempotency-Key
                for each write request
              </li>
              <li>
                <strong>Webhooks:</strong> HMAC‑SHA256 signatures via{" "}
                <code>X-BizPay-Signature</code> with timestamp and replay
                protection
              </li>
            </ul>
            ...
            <ul>
              <li>
                <code>customer</code>, <code>bank_account</code> (tokenized),{" "}
                <code>mandate</code>
              </li>
              <li>
                <code>payment</code> (ach_debit/ach_credit), <code>payout</code>
                , <code>event</code>
              </li>
            </ul>
            ...
            <pre>
              <code>{`# example API key (use your own from the dashboard)
...
curl -s https://api.bizpaysol.com/v1/customers \
  ...
  -d '{
    ...
    "metadata": {"segment": "SaaS"}
            ...
            <pre>
              <code>{`curl -X POST https://api.bizpaysol.com/v1/bank_accounts \
  -H "Authorization: Bearer sk_test_1234567890abcdef1234567890abcd" \
  -H "Content-Type: application/json" \
  -d '{
    ...
    "plaid_public_token": "public-sandbox-abc123xyz"
            ...
            <pre>
              <code>{`curl -X POST https://api.bizpaysol.com/v1/payments \
  -H "Authorization: Bearer sk_test_1234567890abcdef1234567890abcd" \
  ...
  -d '{
    ...
    "description": "Invoice #4821"
            ...
            <pre>
              <code>{`POST /webhooks/bizpay (example payload)
Headers:
  ...
  "data": {
    "payment": {
      ...
      "status": "succeeded"
            ...
            <ul>
              <li>
                400 <code>validation_error</code> • 401{" "}
                <code>unauthorized</code> • 403 <code>forbidden</code>
              </li>
              <li>
                404 <code>not_found</code> • 409{" "}
                <code>idempotency_conflict</code> • 422{" "}
                <code>unprocessable</code>
              </li>
              <li>
                429 <code>rate_limited</code> • 5xx <code>server_error</code>
              </li>
            </ul>
          </div>
        </div>
