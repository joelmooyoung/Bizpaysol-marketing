...
async function generateAnswer(question: string, contexts: { content: string; url?: string; title?: string }[]) {
  const contextText = contexts
    .map((c, i) => `Source ${i + 1}${c.title ? `: ${c.title}` : ''}${c.url ? `\n${c.url}` : ''}\n${c.content}`)
    .join('\n\n');
...
export const handler = async (event: any) => {
  try {
    ...
    const action = (event.queryStringParameters?.action || 'answer') as 'ingest' | 'answer';
    ...
    if (action === 'ingest') {
      ...
      for (const t of texts) items.push({ content: t.content, title: t.title, url: t.url });
      ...
      for (const it of items) {
        ...
        for (const part of parts) {
          ...
          rows.push({ url: it.url, title: it.title, content: part, embedding: e });
        }
      }
    ...
    const matches = await supabaseRPCMatch(qe, Math.min(8, Math.max(3, body.matchCount || 5)));
    const answer = await generateAnswer(q, matches.map((m: any) => ({ content: m.content, url: m.url, title: m.title })));
    return { statusCode: 200, body: JSON.stringify({ answer, sources: matches }) };
  } catch (e: any) {
